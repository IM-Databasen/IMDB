  ...
  <div>
    <form class="mb-3">   
      ...
      <div class="relative">
        ...
        <input type="search" v-model="search" id="default-search" class="block p-4 pl-10 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Søk prøver...">
      </div>
    </form>
    <div class="grid grid-rows-1 md:grid-cols-3">
      <article v-for="item in filteredCards" :key="item.id" class="bg-gray-700 shadow text-white rounded-2xl p-5 mx-2">
        <h1 class="font-bold">
          {{ item.title }}
        </h1>
        <p class="font-light">
